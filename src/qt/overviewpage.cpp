...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CBSL)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount cbslAvailableBalance = balance - immatureBalance - nLockedBalance;

    // CBSL Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // CBSL labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, cbslAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // CBSL Available
    bool showCBSLAvailable = settingShowAllBalances || cbslAvailableBalance != nTotalBalance;
    bool showWatchOnlyCBSLAvailable = showCBSLAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showCBSLAvailable || showWatchOnlyCBSLAvailable);
    ui->labelBalance->setVisible(showCBSLAvailable || showWatchOnlyCBSLAvailable);
    ui->labelWatchAvailable->setVisible(showCBSLAvailable && showWatchOnly);

    // CBSL Pending
    bool showCBSLPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyCBSLPending = showCBSLPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showCBSLPending || showWatchOnlyCBSLPending);
    ui->labelUnconfirmed->setVisible(showCBSLPending || showWatchOnlyCBSLPending);
    ui->labelWatchPending->setVisible(showCBSLPending && showWatchOnly);
    ...
    // CBSL Locked
    bool showCBSLLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyCBSLLocked = showCBSLLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showCBSLLocked || showWatchOnlyCBSLLocked);
    ui->labelLockedBalance->setVisible(showCBSLLocked || showWatchOnlyCBSLLocked);
    ui->labelWatchLocked->setVisible(showCBSLLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("CBSL")
    updateDisplayUnit();
}
