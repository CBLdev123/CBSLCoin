...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Cbslcoin with fAggressive=true to
    // get as much data as possible.
    // Rewrite cbslcoind data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> cbslcoindData;
    bool allOK = dbenv.Cbslcoin(newFilename, true, cbslcoindData);
    if (cbslcoindData.empty()) {
        LogPrintf("Cbslcoin(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Cbslcoin(aggressive) found %u records\n", cbslcoindData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, cbslcoindData) {
        if (fOnlyKeys) {
